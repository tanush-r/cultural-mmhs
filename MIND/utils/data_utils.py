...
def get_item_data_updated(item: dict, dataset_name: str):
    """
    Retrieves image filename, text content, and label from a data item
    based on the dataset's configuration. Handles label mapping.
    """

    # NEW: handle misogyny* datasets (csv style)
    if dataset_name.lower().startswith("misogyny"):
        image_filename = item.get("image_id") or item.get("fimage_id")
        text_content = item.get("transcriptions")
        label = item.get("labels").lower()   # keep as-is

        return image_filename, text_content, label
